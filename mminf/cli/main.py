...
def _repo_root() -> Path:
    # mminf package lives at <repo>/mminf/ ; configs/ sits at <repo>/configs/.
    return Path(mminf.__file__).resolve().parent.parent
...
def _serve(args: argparse.Namespace) -> None:
    ...
    who = getpass.getuser() if hasattr(getpass, "getuser") else "mminf"
    socket_prefix = args.socket_path_prefix or f"/tmp/mminf_{who}/"
    upload_dir = args.upload_dir or f"/tmp/mminf_uploads_{who}/"
    ...
    from mminf.api_server.entrypoint import main as serve_main

    serve_main(argv)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="mminf", description="mminf multimodal inference CLI")
