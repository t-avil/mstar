...
|-----------|-----------|---------|---------|
| Continuous batching (prefill+decode in same step) | YES (default, v1 engine) | NO -- `micro_scheduler.py:234-242` enforces same `graph_walk` per batch | NO -- same constraint |
| Multi-modal prefill schedule | Single-shot prefill | Per-modality walks (prefill\_text, prefill\_audio, prefill\_vision) scheduled as separate micro\_scheduler steps | Single-shot prefill |
| KV cache block/page size | 16 (`vllm/config/cache.py:45`) | 128 (`mstar/engine/kv_store.py:77`) | 128 |
...
| Code2Wav CUDA-graph | Eager only | Captured (bs={1,2,4,8,16,32}) | Captured |

Per-token IPC cost for I2S = same as S2S (Stage 0 -> Stage 1 -> Stage 2 = two cross-process boundaries per audio token).
