...
ROOT = sys.argv[1] if len(sys.argv) > 1 else _BENCH_DIR
OUT  = sys.argv[2] if len(sys.argv) > 2 else os.path.join(ROOT, "charts")
for _s in [os.path.join(_BENCH_DIR, "..", "chartstyle.mplstyle"),
           os.path.join(os.path.dirname(__file__), "chartstyle.mplstyle")]:
...
SYS = [("mstar_new", "M*-new (native encoders)", "#1f77b4", "o", "-"),
