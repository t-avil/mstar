...
> ITL note: M\*-old per-token latency at higher batch on S2T (B=2/8/16 ITL_mean =
> 0.0007/0.0005/0.0002 s) is a sub-millisecond artifact, not a real per-token latency —
> the old serialized path bursts all tokens after a multi-second queue, so its measured
> inter-token gap collapses. The values are shown in NUMBERS.md for completeness; the
> B=1 (no-batching) comparison is the meaningful ITL comparison.
...
**PASS** — M\*-new throughput exceeds M\*-old at all 24 test points (4 paths x 6 batches;
strictly greater on every point).
