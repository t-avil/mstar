...
# Build: opt/prep-pos-batched-v9 @915ab8f (E1) = opt/prep-h2d @620de91
#        + MSTAR_PREP_DEVICE_POS_BATCHED (batched pos_ids H2D fix, +5.9% B32 same-server A/B). Env adds CDT cache.
...
exec bash /m-coriander/coriander/tim/lab_server.sh best /m-coriander/coriander/tim/mstar-godv9 "TORCHINDUCTOR_FX_GRAPH_CACHE=1 TORCHINDUCTOR_CACHE_DIR=/m-coriander/coriander/tim/inductor_cache_cdt TORCHINDUCTOR_COORDINATE_DESCENT_TUNING=1 TORCHDYNAMO_CACHE_SIZE_LIMIT=128 MSTAR_CUSTOM_OPS=1 MSTAR_MOE_FP8=1 MSTAR_BATCH_EMIT=1 MSTAR_FAST_POSTPROC=1 MSTAR_CHUNKED_PREFILL_V2=1 MSTAR_MIXED_BATCH=1 MSTAR_MIXED_SPEC=1 MSTAR_PREFILL_CHUNK_TOKENS=512 MSTAR_SLIM_EMIT=1 MSTAR_SLIM_EMIT2=1 MSTAR_FAST_ROUTE=1 MSTAR_FAST_ROUTE2=1 MSTAR_SAMPLER_CFG_CACHE=1 MSTAR_SAMPLER_CFG_CACHE_V2=1 MSTAR_FAST_CHECKSTOP=1 MSTAR_FAST_SEND=1 MSTAR_EMIT_SIDECAR=1 MSTAR_SIDECAR_CHECKSTOP=1 MSTAR_MIXED_BUDGET_TOKENS=512 MSTAR_FAST_CHECKSTOP_TALKER=1 MSTAR_CODEC_CHUNK_EMIT=1 MSTAR_MERGED_PREFILL=1 MSTAR_PREP_DEVICE_POS=1 MSTAR_PREP_DEVICE_POS_BATCHED=1" configs/qwen3omni_2gpu_encoff.yaml "$GPUS" "$PORT" "$TTL"
