...
# Use Inductor's persistent default cache ($TMPDIR/torchinductor_$USER) so warm
# kernels are REUSED across boots. (An explicit empty CACHE_DIR here discarded the
# warm cache and forced a ~25 min full recompile every boot.)
COMPILE="TORCHINDUCTOR_FX_GRAPH_CACHE=1 TORCHDYNAMO_CACHE_SIZE_LIMIT=128"
