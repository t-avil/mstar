#!/usr/bin/env python3
"""Layer-by-layer diff of mstar's Pi0.5 forward path against lerobot.
...
component-level test: if mstar's components match lerobot here but the server
diverges, the bug is in the server-only path (FlashInfer cache, dtype,
data_worker preprocessing). If mstar already diverges here, the bug is in
...
# ----------------------------------------------------------------------------
# mstar in-process: load Pi05Model via the same remapper the server uses.
# ----------------------------------------------------------------------------
...
def load_mstar(ref_model, ref_config, device):
    ...
    print(f"loading mstar weights from {safetensors_path}")
...
def main():
    ...
    mstar_model = load_mstar(ref_model, ref_config, device)

    # ----- 2. Build inputs -----
    images_pil, mstar_images = build_inputs(args.seed, device)
    img_masks_lerobot = [torch.ones(1, dtype=torch.bool, device=device) for _ in mstar_images]

    # Build tokens via the SAME tokenizer mstar uses (PaliGemma fast).
    ...
    print(f"image count = {len(mstar_images)}")
    ...
    with torch.no_grad():
        ...
        # mstar: run Pi05ViTEncoderSubmodule.preprocess + forward on the SAME [-1,1] inputs
        ...
        per_request_inputs: list[NameToTensorList] = [
            {
                "image_inputs": [
                    torch.stack([img.squeeze(0) for img in mstar_images], dim=0)
        ...
        mstar_img_features = vit_submodule.forward(
            request_info=info, **prep
        )["img_emb"][0]  # [num_cams * 256, 2048] — UNSCALED (sqrt happens in LLM submodule)

    # Compare per-camera. mstar img_emb is unscaled; lerobot ref_per_cam is scaled
    # by sqrt(hidden) in embed_image.
    image_scale = mstar_model.config.hidden_size**0.5
    mstar_per_cam_scaled = (
        mstar_img_features.reshape(3, 256, -1) * image_scale
    )
    ref_per_cam_stacked = torch.cat([r for r in ref_per_cam], dim=0)  # [3, 256, 2048]
    stat("vit_encoder per-cam scaled", mstar_per_cam_scaled, ref_per_cam_stacked, tol=args.tol)
    ...
    with torch.no_grad():
        ...
        ref_prefix_embs, ref_prefix_pad_masks, _ = ref_model.embed_prefix(
            [i.to(dtype) for i in mstar_images], img_masks_lerobot, tokens, masks_lerobot
        )
        ref_prefix = ref_prefix_embs[0]

        # mstar _preprocess_prefill, with the freshly-encoded img_emb above.
        per_request_inputs_llm: list[NameToTensorList] = [
            {
                "img_emb": [mstar_img_features],
    ...
    n_layers = mstar_model.config.num_layers
    ...
    with torch.no_grad():
        # mstar full denoise loop using the captured prefix KV (mock cache).
        suffix_positions = (
        ...
        ref_actions = ref_model.sample_actions(
            images=[i.to(dtype) for i in mstar_images],
