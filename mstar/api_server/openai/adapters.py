...
class SubmitArgs:
    ...
    # Ordered text/attachment sequence; None for entrypoints that carry no
    # ordering, which fall back to the legacy attachments-then-text layout.
...
def flatten_messages(
    ...
    ``parts`` is the ordered sequence of text and attachments as written, and
    the other three are derived from it: text newline-joined, attachments
    persisted under ``upload_dir`` and grouped by modality, and
    ``input_modalities`` the per-part modality sequence — so an attachment's
    position and a repeated modality both survive. (Multi-turn role structure
    is flattened — a v1 simplification; the models here apply their own prompt
    formatting in ``process_prompt``.)
    """
